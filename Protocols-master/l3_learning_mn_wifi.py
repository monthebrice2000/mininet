...
class l3_switch(EventMixin):
    ...
    """
        if (dpid, ipaddr) in self.lost_buffers:
            ...
            print(" bucket ", bucket, " ", len(bucket), " ", dpid_to_str(dpid) )
            ...
            for _, buffer_id, in_port in bucket:
                ...
                #core.openflow.sendToDPID(dpid, po)
                core.openflow.getConnection(dpid).send(po)
        ...
        if isinstance(packet.next, ipv4):
            ...
            if dstaddr in self.arpTable[dpid]:
                ...
                if prt == inport:
                    ...
                    if self.wide:
                        print( "corrrespondance ")
                        match = of.ofp_match(dl_type=packet.type, nw_dst=dstaddr, nw_src=packet.next.srcip)
                    else:
                        print( "correspondance sur le paquet ")
                ...
                e = ethernet(type=ethernet.ARP_TYPE, src=packet.src,
                             dst=ETHER_BROADCAST)
                e.set_payload(r)
                print(" dpid ", dpid, " inport", inport, " str(r.protodst)", str(r.protodst), " str(r.protosrc)",
                      str(r.protosrc))
                            ...
                            if a.protodst in self.arpTable[dpid]:
                                ...
                                if not self.arpTable[dpid][a.protodst].isExpired():
                                    ...
                                    log.debug("%i %i answering ARP for %s" % (dpid, inport,
                                                                              r.protosrc))
                                    print("arp response dpid ", dpid, "  numero de l'ap ", inport," str( r.protosrc ) ", str(r.protosrc))
                                    msg = of.ofp_packet_out()
                                    msg.data = e.pack()
