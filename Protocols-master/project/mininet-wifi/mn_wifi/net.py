...
class Mininet_wifi(Mininet, Mininet_IoT):
    ...
    def __init__(self, accessPoint=OVSKernelAP, station=Station, car=Car,
                 ...
                 hostToExchangeData="0.0.0.0", portToExchangeData=1037, **kwargs):
        ...
        self.hostToExchangeData = hostToExchangeData
        self.portToExchangeData = portToExchangeData

        print('Waiting for connection response')
        try:
            self.ClientMultiSocket.connect((self.hostToExchangeData, self.portToExchangeData))
    ...
    def f1(self, node):
        response = self.ClientMultiSocket.recv(1024) 
        
        if response is None:
            return
        elif response.decode('utf-8') == "Hello !":
            capacity = node.capacity
            print("capacity: {}".format(capacity))
            residualEnergy = node.wintfs[0].residualEnergy
            print("residualEnergy: {}".format(residualEnergy))
            node_name = node.name;
            print( "Ap name : {}".format( node_name ) )
            self.ClientMultiSocket.send((str(residualEnergy)+","+str(capacity)+","+str(node_name)).encode('utf-8'))
        else:
            return
    ...
    def hasVoltageParam(self):
        info("*** start HasVoltageParam function\n")
        ...
        #     for node in nodes:
        #         if 'voltage' in node.params:
        #             print(node.wintfs.values())
        #             #Ajout fonction pour envoie d'informations au controller
        #             info("*** Envoie d'informations de {} \n".format(node.name) )
        #             self.f1(node)
        #
        # self.ClientMultiSocket.close()
        info("*** end HasVoltageParam function\n")
