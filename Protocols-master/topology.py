...
class OpenFlowTopology (object):
  ...
  def _handle_openflow_discovery_LinkEvent (self, event):
    ...
    """
    #je recupere un lien venant la topologie actuelle
    link = event.link
    #je recupere l'entite  du premier switch a travers le premier port
    sw1 = self.topology.getEntityByID(link.dpid1)
    #je recupere l'entite du second switch a travers le second port
    sw2 = self.topology.getEntityByID(link.dpid2)
    #si l'entite pour l'un des switch est nulle sortir de la fonction
    if sw1 is None or sw2 is None: return
    #on verifie si les deux ports portés par les liens sont sur dans l'un des ports des switchs sinon on sort de la fonction
    if link.port1 not in sw1.ports or link.port2 not in sw2.ports: return
    #verifier si l'event est creer ou en cours
    if event.added:
      print( "sw1 avant --> ", dpidToStr( link.dpid1 ) )
      print("sw2 apres--> ", dpidToStr( link.dpid2 ) )
      print( "-------------------------------")
      #pour ajouter une entite du switch 2 au port du swhitch 1
      sw1.ports[link.port1].addEntity(sw2, single=True)
      #pour ajouter une entiter du switch 1 au port du switch 2
      sw2.ports[link.port2].addEntity(sw1, single=True)
    elif event.removed:
      #pour supprimer une entite du switch 2 sur le port du switch 1
      sw1.ports[link.port1].entities.discard(sw2)
      #pour supprimer une entite du switch 1 sur le port du switch 2
      sw2.ports[link.port2].entities.discard(sw1)

  def _handle_openflow_ConnectionUp (self, event):
    #je recuoere l'entite du switch par son identifiant
    sw = self.topology.getEntityByID(event.dpid)
    #je suppose que l'ajout du nouveau switch n'a pas ete effectué
    add = False
    #je verifie si l'entite du switch nulle
    if sw is None:
      #je cree une instance du switch openflow
      sw = OpenFlowSwitch(event.dpid)
      #jje notifie que l'ajout du nouveau switch
      add = True
    else:
      #je verifie si le switch a une connexion
      if sw._connection is not None:
        log.warn("Switch %s connected, but... it's already connected!" %
                 (dpidToStr(event.dpid),))
    #je met a jour les donnees de connexion de l'entite switch openflow
    sw._setConnection(event.connection, event.ofp)
    log.info("Switch " + dpidToStr(event.dpid) + " connected")
    #je verifie si je peux notifier l'ajout d'un nouveau switch dans la topology
    if add:
      #j'ajoute la nouvelle entite dans la topologie
      self.topology.addEntity(sw)
      #je publie un evenement  SwitchJoin sur l'entite
